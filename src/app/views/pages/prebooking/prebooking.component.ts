...
export class PrebookingComponent implements OnInit, OnDestroy {
      ...
      this.api.AirBookingGetDataDB(this.sessionID).subscribe((AirBookingGetDataDB: any) => {
        ...
        this.api.AirPricePort(this.sessionID).pipe(
          tap((AirPricePort: any) => {
            ...
            this.airPricingInfoKey = AirPricePort.data[0].airPricingInfoKey;
  ...
  submitBook() {
    ...
    this.api.AirCreateReservationPort(this.sessionID, dataBooking.title, dataBooking.firstname, dataBooking.lastname, dob, dataBooking.email, dataBooking.phone, this.submitedPassengerData, this.supplier, this.airPricingInfoKey).pipe(
      tap((data: any) => {
        if (data.status == 1) {
  ...
  submitPayment() {
    ...
    this.api.insertPaymentChannelEspay(this.bookingID, dataBooking.bankCode, this.origin, this.destination, departureTimeModified, this.airPlane, this.roundType, this.airPricePort).pipe(
      tap((data: any) => {
        if (data.status == 1) {
          this.stepPayComplete = true;
          this.hitAPICheckPayment();
